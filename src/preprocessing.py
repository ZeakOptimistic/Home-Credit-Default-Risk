...
def prepare_main_table_with_bureau_and_previous(
    train,
    test,
    bureau_features,
    previous_features,
    ...
    if not previous_features[ID_COL].is_unique:
        raise ValueError("previous_features must be unique at SK_ID_CURR level")

    train_merged = (
        train.merge(bureau_features, on=ID_COL, how="left")
             .merge(previous_features, on=ID_COL, how="left")
    )
    test_merged = (
        test.merge(bureau_features, on=ID_COL, how="left")
            .merge(previous_features, on=ID_COL, how="left")
    )
